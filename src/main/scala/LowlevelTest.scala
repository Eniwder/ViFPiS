...
object LowlevelTest extends App {
  ...
  val jvmOptions = Seq("-classpath", s"F:/Temp/ViFPiS/target/scala-2.11/classes;$classpath")
  ...
  val file = Source.fromFile("F:/Temp/ViFPiS/src/main/scala/DebugTarget.scala").getLines().toList
      ...
      if (caller.contains(listClass)) {
        ...
        if (collectionExpNow) {
          if (collectionMethodInit) {
            ...
            val lambdaRegex1 = """.*?\w+? \w+? \{(.*?)=>(.*?)\}.*""".r // xxx = { (x:Int) => x * 2}
            val lambdaRegex2 = """.*? \w\.\((.*?)=>(.*?)\).*""".r // xxx = (x:Int) => x*2
            val lambdaRegex3 = """.*?\{(.*?)=>(.*?)\}.*""".r // { x => x*2 }
            val lambda = lambdaExpSource match {
                case lambdaRegex1(arg, exp) => (arg,exp)
                case lambdaRegex2(arg, exp) => (arg,exp)
                case lambdaRegex3(arg, exp) => (arg,exp)
                case _                      => ("err","lambdaExpSource")
              }
            collectionMethodInit = false
            println(CMList,CMName,CollectionLambda(lambda._1.trim,lambda._2.trim))
          }
